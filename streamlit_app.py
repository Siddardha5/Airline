...
# Define the positive experience chain as a RunnableLambda
positive_chain = RunnableLambda(
    lambda inputs: llm(PromptTemplate.from_template(
        ...
        Text:
        {feedback}
        """).format(feedback=inputs["feedback"]))
)

# Define the negative experience chain for issues caused by the airline
negative_airline_fault_chain = RunnableLambda(
    lambda inputs: llm(PromptTemplate.from_template(
...
negative_not_airline_fault_chain = RunnableLambda(
    lambda inputs: llm(PromptTemplate.from_template(
        """You are a professional customer service representative.
